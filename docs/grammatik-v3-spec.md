...
8. **Schritt 4A GEBAUT + ABGENOMMEN (16.08. nachts):** `asset.subs` (Sub-Zeile je Label-Platz, `subMax` JE ROLLE gemessen — inline staucht, ein Rollen-Minimum hätte das abgenommene Mockup verboten) + `notes[]` auf asset-Karten (max 2, Umbruch macht der Renderer, Ton via style vererbbar) — beides additiv. **LEER umgehängt:** audit-lesson klassifiziert (`ZÄHLUNG befunde/leer/system` + `HART-LEER` mit Korrektur), glm-generate entscheidet die Schuld — reines LEER → Generator-Patch-Runde → Re-Audit (beide Wege am Stub bewiesen: exit 0 / reject-audit exit 1); `system>0` → exit 3 (Zweig UNGETESTET, Fixture fehlt — offener Punkt). **Note-Punkt-Regel (Leon-Abnahme, ersetzt Mockup-Handposition):** der Punkt liegt auf der GEZEICHNETEN Geometrie seines Ankers — wie der curve-Note-Punkt auf seiner Kurve; keine Sonderregel für Flächen-Anker. Generator-Prompt: Registry wird beim Start aus dem Manifest gebaut (Anker-Mismatch bricht laut), Judge-Zwangsauftrag `begriffs-budget` je Asset-Karte.
