...
7. **Begriff-Aufträge** (kind „begriff-greifbar"): Diagramm-Begriffe (Serien-Labels, Noten, Ereignis-Label, Achsen) müssen Erlebnis-/Alltagssprache sein — ein Leser ohne Fachwissen greift sie sofort. Fehler ist NUR: (a) Fach-/Stoffvokabular auf dem Graphen, das ein Laie nicht fühlt („Adenosin", „Kortisol", „Rezeptor" — der Fachbegriff gehört in den Lehrsatz, auf den Graphen seine greifbare Übersetzung wie „Echte Müdigkeit"/„Was du spürst"), oder (b) ein Noten-Label, das sinngemäß ein Serien- oder Ereignis-Label derselben Karte wiederholt (Redundanz ist Lärm). Alltags- und Themenwörter (Koffein, Kaffee, Gehalt, Zinsen) sind NIE Fehler; Kürze oder CAPS sind NIE Fehler; der Lehrsatz wird hier nicht beurteilt. fix = konkrete Umbenennung im Längen-Limit des Felds; bei redundanter Note fix mit path cards[i].notes und dem vollständigen bereinigten Array. Verdict „wrong" nur bei klarer Verletzung, sonst ok.
8. **Objekt-Karten-Aufträge** (kind „begriffs-budget"): Der Auftrag zählt alle Texte auf, die IM Bild einer Objekt-Karte stehen (Label, Sub-Zeilen, Notes). Beurteile zwei Dinge und schreibe beide in die Rechnung: (a) Wort-Sinn wie in Regel 5, (b) Redundanz und Fachsprache — sagt jeder Text etwas Eigenes aus der Sicht des Lesers? Eine Sub-Zeile, die ihr Label nur umformuliert („WAS DU ZEIGST" / „WAS ANDERE SEHEN"), und eine Note, die ein Label wiederholt, sind Lärm im Bild → „wrong" mit fix (der bessere Text, Längen-Limit des Felds beachten; path z. B. cards[3].asset.subs.aussen oder cards[3].notes[0].text). Dichte allein ist KEIN Fehler: zwei Sub-Zeilen und zwei Notes sind erlaubt, solange jede etwas Eigenes trägt. Position, Größe und Platzierung beurteilst du nie — die macht das System.
